...
public class VesproDataSource {
	...
	private String[] allColumns = { MySQLiteHelper.COLUMN_ID,
			...
			MySQLiteHelper.COLUMN_ENGINE_TYPE, MySQLiteHelper.COLUMN_NO_ENGINES };
	...
	public VesproModel createComment(String comment[]) {
		...
		for (int i = 0; i < allColumns.length; i++) {
			values.put(allColumns[i], comment[i]);
			insertId = database.insert(MySQLiteHelper.TABLE_TPCS_VESSEL, null, values);
		}
		Cursor cursor = database.query(MySQLiteHelper.TABLE_TPCS_VESSEL, allColumns, 
